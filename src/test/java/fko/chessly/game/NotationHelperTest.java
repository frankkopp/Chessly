...
//    assertTrue(m17 != null);
   ...
   * square>]['x']<to square> <SAN move descriptor pawn captures> ::= <from file>[<from rank>] 'x'
   * <to square>[<promoted to>] <SAN move descriptor pawn push> ::= <to square>[<promoted to>]
   *
   * <p>Ambiguities If the piece is sufficient to unambiguously determine the origin square, the
   * whole from square is omitted. Otherwise, if two (or more) pieces of the same kind can move to
   * the same square, the piece's initial is followed by (in descending order of preference) 1. file
   * of departure if different 2. rank of departure if the files are the same but the ranks differ
   * 3. the complete origin square coordinate otherwise
   *
   * <p>Captures Captures are denoted by the lower case letter "x" immediately prior to the
   * destination square. Pawn captures with the omitted piece symbol, include the file letter of the
   * originating square of the capturing pawn prior to the "x" character, even if not required for
   * unambiguousness. Some SAN variations in printed media even omit the target rank if unambiguous,
   * like dxe, which might not be accepted as input format.
   *
   * <p>En passant The PGN-Standard does not require En passant captures have any special notation,
   * and is written as if the captured pawn were on the capturing pawn's destination square.FIDE
   * states the redundant move suffix "e.p." optional (after 1 July 2014) [11].
   *
   * <p>In the case of an ‘en passant’ capture, ‘e.p.’ may be appended to the notation.
   *
   * <p>Pawn promotion A pawn promotion requires the information about the chosen piece, appended as
   * trailing Piece letter behind the target square. The SAN PGN-Standard requires an equal sign
   * ('=') immediately following the destination square.
   *
   * <p>Castling Castling is indicated by the special notations, "O-O" for kingside castling and
   * "O-O-O" for queenside castling. While the FIDE handbook [12] uses the digit zero, the SAN
   * PGN-Standard requires the capital letter 'O' for its export format.
   *
   * <p>Converting Moves Due to the most compact representation, considering ambiguities concerning
   * the origin square, converting moves with pure from- and to-squares to SAN requires not only an
   * underlying board representation to determine piece initials, but also legal move generation for
   * a subset of moves to the destination square. Pseudo legal, but illegal moves for instance with
   * a Pinned piece must not be considered in ambiguous issues in an export format.
   */
  @SuppressWarnings("unused")
  @Test
...
//    assertTrue(m17 != null);
  ...
  public void testMoveFromOldSimpleNotation() {
    ...
    GameMove m1 =
        NotationHelper.createNewMoveFromSimpleNotation("e2-e4", Pawn.create(GameColor.WHITE));
    GameMove m1twin =
        NotationHelper.createNewMoveFromSimpleNotation("e2-e4", Pawn.create(GameColor.WHITE));
    GameMove m1copy = new GameMoveImpl(m1);
    GameMove m2 =
        NotationHelper.createNewMoveFromSimpleNotation("d2-d4", Pawn.create(GameColor.WHITE));
    GameMove m3 =
        NotationHelper.createNewMoveFromSimpleNotation("e2-e3", Pawn.create(GameColor.WHITE));
    GameMove m4 =
        NotationHelper.createNewMoveFromSimpleNotation("a2a4", Pawn.create(GameColor.WHITE));

    System.out.println("Move Orig: " + m1.toString() + ": " + m1.hashCode());
    System.out.println("Move Twin: " + m1twin.toString() + ": " + m1twin.hashCode());
    System.out.println("Move Copy: " + m1copy.toString() + ": " + m1copy.hashCode());
